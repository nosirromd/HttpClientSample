...
namespace HttpClientSample
{
    public class HttpClientSamples : IDisposable
    {
        ...
        private const string ImgurApiImagesUrl = "https://api.imgur.com/3/account/me/images";
        ...
        public async Task GetDataWithHeadersAsync()
        {
            ...
            {
                HttpClient.DefaultRequestHeaders.Add("Accept", "application/json;odata=verbose");
                HttpClient.DefaultRequestHeaders.Add("Authorization", "Bearer 7b2f9a0efaba977dbcf77a4cc7db5ccc92e9222a");
                ShowHeaders("Request Headers:", HttpClient.DefaultRequestHeaders);

                HttpResponseMessage response = await HttpClient.GetAsync(ImgurApiImagesUrl);
                response.EnsureSuccessStatusCode();
